...
class Table extends React.Component {
  constructor(props) {
    super(props);
    gameState.tableID = props.tableID;
    const portNumber = 8000 + props.tableID;
    console.log('portNumber:', portNumber);
    socket = new WebSocket(`ws://localhost:${portNumber}`);
    this.state = gameState;
    console.log('this.state.gameState:', this.state);
    ...
    socket.addEventListener('open', () => {
      ...
      socket.addEventListener('message', serverMessage => {
        ...
        if (typeof parsedSM === 'number')
          this.setState({
            playerCount: parsedSM
          });
        else {
          const { p1, p2 } = parsedSM;
          if (parsedSM.bettingRound === 5)
            parsedSM.deal = false;
          if (p1 && p2)
            parsedSM.betSelect = eval(`p${this.state.playerCount}`).minBet;
  ...
  changeBet() {
    this.setState({
      betSelect: Number(event.target.value)
    });
  };
  ...
  clicker() {
    ...
    if (event.target.value === 'fold')
      socket.send(JSON.stringify(fold(playerCount, bettingRound, currentBets, pot, p1, p2)));
    else {
